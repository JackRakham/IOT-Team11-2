...
"""
def get_station_data(request, **kwargs):
    

    stationIdParam = kwargs.get("station", None)
    
    StationObject = Station.objects.filter(id=stationIdParam)[0]

    if StationObject is None:
         return JsonResponse({
    ...
    stationdata = Data.objects.filter(station_id= stationIdParam)
    data = []

    minVal = stationdata.aggregate(Min("value"))["value__min"]
    maxVal = stationdata.aggregate(Max("value"))["value__max"]
    avgVal = stationdata.aggregate(Avg("value"))["value__avg"]

    data.append(
        {
            "name": f"{StationObject.location.city.name}, {StationObject.location.state.name}, {StationObject.location.country.name}",
            "lat": StationObject.location.lat,
            "lng": StationObject.location.lng,
            "last activity": StationObject.last_activity,
            "min": minVal if minVal != None else 0,
            "max": maxVal if maxVal != None else 0,
            "avg": round(avgVal if avgVal != None else 0, 2),
        }
